...
class AuthController {
  ...
  async check(req, res, next) {
    try {
      ...
      All.query(
        `SELECT * FROM sb_admins WHERE authid LIKE "%${steam_short}%"`,
        function (err, results) {
          ...
          if (typeof results !== "undefined") {
            ...
            if (typeof group !== "undefined") {
              const groups_levels = JSON.parse(
                fs.readFileSync("./config/groups.json", "utf8")
              );
              if (groups_levels[group]) group_return = groups_levels[group];
            }
          }
          const token = generateJwt(
            user.username,
